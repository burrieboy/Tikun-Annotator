...
SCORE_TO_NUM_GAP     = 15   # Constant spacing between Scores and Line Numbers
NUM_TO_CATCH_GAP     = 12   # Constant spacing between Line Numbers and Catchwords
...
# =========================================================================
def generate_annotated_tikun_streamlit(uploaded_file, output_buffer, gap_from_text, arrow_shift):
    ...
    for page_num in range(total_pages):
        ...
        for block in page_data.get("blocks", []):
            if "lines" in block:
                for line in block["lines"]:
                    ...
                    if not any('\u0590' <= char <= '\u05fe' for char in cleaned_physical): continue
                        
                    raw_lines.append({
                        "text": line_text, "chars": all_chars, "spans": line["spans"], 
                        "bbox": line["bbox"], "center_y": center_y, "x0": x0
                    })
        ...
        for r_line in raw_lines:
            ...
            if not merged:
                consolidated_lines.append({
                    "parts": [r_line], "bbox": r_line["bbox"], "center_y": r_line["center_y"]
                })
        
        valid_blocks = []
        for c_line in consolidated_lines:
            all_chars = []
            for part in c_line["parts"]: all_chars.extend(part["chars"])
            printable_chars = [c for c in all_chars if c["c"].strip()]
            if not printable_chars: continue
            for c in printable_chars: c["is_biblical"] = True
            ...
            for char_obj in biblical_chars:
                if not current_cluster: current_cluster.append(char_obj)
                else:
                    ...
                    else: current_cluster.append(char_obj)
            if current_cluster: biblical_word_clusters.append(current_cluster)
                
            biblical_words = []
            for cluster in biblical_word_clusters:
                cluster.sort(key=lambda c: c["bbox"][0], reverse=True) 
                word_text = "".join([c["c"] for c in cluster])
                if word_text.strip(): biblical_words.append(word_text)
            ...
            if current_cluster: physical_word_clusters.append(current_cluster)
                
            physical_words = []
            for cluster in physical_word_clusters:
                cluster.sort(key=lambda c: c["bbox"][0], reverse=True) 
                word_text = "".join([c["c"] for c in cluster])
                if word_text.strip(): physical_words.append(word_text)
            
            cleaned_biblical_words = [re.sub(r'[^\u05d0-\u05ea\u0590-\u05c7]', '', re.sub(r'^\d+\s*', '', w).strip()).strip() for w in biblical_words]
            cleaned_biblical = " ".join(list(reversed([w for w in cleaned_biblical_words if w])))
            
            cleaned_physical_words = [re.sub(r'^\d+\s*', '', w).strip() for w in physical_words]
            cleaned_physical = " ".join(list(reversed([w for w in cleaned_physical_words if w])))
            
            if not cleaned_physical or cleaned_physical.isdigit() or len(cleaned_biblical) == 0 or len(cleaned_physical.split()) < 2:
                continue
            ...
            if digit_match:
                ...
                for char_obj in combined_chars:
                    if char_obj["c"].isdigit():
                        ...
                        if inline_num_rect is None: inline_num_rect = fitz.Rect(char_obj["bbox"])
                        else: inline_num_rect.include_rect(char_obj["bbox"])
            ...
            for cluster in biblical_word_clusters:
                ...
                if clean_word == "יהוה": hashem_clusters.append(cluster)
            
            hashem_char_ids = {id(c) for cluster in hashem_clusters for c in cluster}
            effective_char_count = len([c for c in hebrew_biblical_chars if id(c) not in hashem_char_ids]) + (4.0 * len(hashem_clusters))
            
            full_line_raw_text = "".join([re.sub(r'[^\u05d0-\u05ea]', '', strip_nikud(c["c"])) for c in hebrew_printable_chars])
            if "אשריאשריאשרי" in full_line_raw_text:
                effective_char_count += 48.0
            else:
                for c in hebrew_printable_chars:
                    if not c.get("is_biblical", True): effective_char_count += 1
            
            valid_blocks.append({
                "physical_text": cleaned_physical, "biblical_text": cleaned_biblical,
                "biblical_words_list": list(reversed([w for w in cleaned_biblical_words if w])),
                "chars": combined_chars, "bbox": c_line["bbox"],
                "inline_num_rect": inline_num_rect, "inline_num_str": inline_num_str,
                "effective_char_count": effective_char_count
            })
        
        valid_blocks.sort(key=lambda b: b["bbox"][1])
        biblical_lines = valid_blocks
        if not biblical_lines: continue
            
        for block in biblical_lines:
            x0, y0, x1, y1 = block["bbox"]
            line_center_y = (y0 + y1) / 2
            num_str, num_rect = block["inline_num_str"], block["inline_num_rect"]
            if not num_str and digit_spans:
                ...
                for span in digit_spans:
                    ...
                    if dist < 20 and dist < best_dist:
                        ...
                        best_match = span
                if best_match:
                    num_str, num_rect = best_match["text"], best_match["bbox"]
                    digit_spans.remove(best_match)
            block["matched_num_str"], block["matched_num_rect"] = num_str, num_rect
            
        max_x1 = max(b["bbox"][2] for b in biblical_lines)
        SCORE_COL_X     = max_x1 + gap_from_text
        ...
        for block in biblical_lines:
            if block["matched_num_rect"]:
                padded_rect = fitz.Rect(block["matched_num_rect"].x0 - 2, block["matched_num_rect"].y0 - 2, block["matched_num_rect"].x1 + 2, block["matched_num_rect"].y1 + 2)
                redactions_to_apply.append(padded_rect)
        for rect in redactions_to_apply: page.add_redact_annot(rect, fill=(1, 1, 1))
        if redactions_to_apply: page.apply_redactions(images=fitz.PDF_REDACT_IMAGE_NONE)
        ...
        for i, block in enumerate(biblical_lines):
            ...
            catch_word = block["biblical_words_list"][0] if block["biblical_words_list"] else "—"
            prev_first_word = catch_word # Note: Logic updated to track per line
            
            page.insert_text(fitz.Point(SCORE_COL_X, baseline_y), fix_rtl(score_str), fontsize=10, fontname="alef", fontfile=font_file, color=(0, 0, 0), overlay=True)
            page.insert_text(fitz.Point(CATCHWORD_COL_X, baseline_y), fix_rtl(catch_word), fontsize=9, fontname="alef", fontfile=font_file, color=(0.4, 0.4, 0.4), overlay=True)
            if block["matched_num_str"]:
                page.insert_text(fitz.Point(LINE_NUM_COL_X, baseline_y), block["matched_num_str"], fontsize=NUMBER_FONT_SIZE, fontname="alef", fontfile=font_file, color=NUMBER_COLOR, overlay=True)
            
            biblical_chars_sorted = sorted([c for c in block["chars"] if c.get("is_biblical", True) and c["c"].strip()], key=lambda c: c["bbox"][0])
            target_char = next((c for c in biblical_chars_sorted if c["c"] in stretchable_letters), None)
            
            if target_char:
                cx0, cy0, cx1, cy1 = target_char["bbox"]
                char_center_x = (cx0 + cx1) / 2
                arrow_tip_y = cy0 + arrow_shift
                arrow_top_y = arrow_tip_y - 4.5
                page.draw_line(fitz.Point(char_center_x, arrow_top_y), fitz.Point(char_center_x, arrow_tip_y), color=(0.8, 0.1, 0.1), width=1)
                page.draw_line(fitz.Point(char_center_x - 1.2, arrow_tip_y - 1.5), fitz.Point(char_center_x, arrow_tip_y), color=(0.8, 0.1, 0.1), width=1)
                page.draw_line(fitz.Point(char_center_x + 1.2, arrow_tip_y - 1.5), fitz.Point(char_center_x, arrow_tip_y), color=(0.8, 0.1, 0.1), width=1)
